...
def load_image_label(label_file):
    label_dir = os.path.dirname(label_file)
    
    def deduce_image_root(image_file):
        root = label_dir
        while True:
    ...
    with open(label_file, encoding='utf-8') as f:
        ...
        for line in f:
            ...
            if image_root is None:
                image_root = deduce_image_root(parts[0])
                assert(image_root is not None, 'no image root duduced')
            image_files.append(os.path.join(image_root, parts[0]))
            labels.append(parts[1])
        return image_files, labels                    

def main(args):
    if args.image_dir.endswith('.txt'):
        image_file_list, _ = load_image_label(args.image_dir)
    else:
    ...
    def do_rec(img_list):
        try:
            ret, _ = text_recognizer(img_list)
            rec_res.extend(ret)
    ...
    for i, image_file in enumerate(image_file_list):
        ...
        img_list.append(img)
        if len(img_list) == batch_size or i == len(image_file_list) - 1:
            do_rec(img_list)
            img_list = []

    for ino in range(len(valid_image_file_list)):
        print(
